import React from 'react';
import { connect } from 'react-redux';
import FileDrop from '../../components/FileDrop';
import Styled from 'styled-components';
import TitleRibbon from '../../components/TitleRibbon';
import Wrapper from '../../components/Wrapper';
import EditContext from '../../utils/EditContext';
import { AddEducation, EditEducation, DeleteEducation } from './actions';

const AvatarImage = Styled.img`
    border-radius: 50% !important;
`;

const StyledComponent = Styled.div`
    display: flex;
    flex-direaction: row;
    align-items: center;
`;

const StyledDiv = Styled.div`
    display: flex;
    flex-direction: row;
    justify-content: space-between;
`;

class Education extends React.Component {

    static contextType = EditContext;

    constructor(props) {
        super(props);
        this.state = { education: [ ], files:[] };
        var today = new Date();
        var dd = String(today.getDate()).padStart(2, '0');
        var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
        var yyyy = today.getFullYear();
        this.defaultEducation = {
            id: 1,
            title: 'school/collage',
            from: '1997-11-10',
            to: `${yyyy}-${mm}-${dd}`,
            description: 'Hey there, Tell something about your Education. try to  keep it within 500 chars',
            files: []
        };
    }

    addGrade = () => {
        this.props.AddEducation(this.defaultEducation);
    }

    addFile = file => {
        console.log(file);
        this.setState({
          files: file.map(file =>
            Object.assign(file, {
              preview: URL.createObjectURL(file)
            })
          )
        });
      };

    renderEditableEducation = () =>{
        return this.props.Education.education.map((item,index) => {
            return <StyledDiv>
                    <form className="ui form" key={index}>
                    <img className="ui small image" src="data:image/png;base64,"></img>
                    <div className="ui input focus">
                        <input type="text" onChange={(e) => { this.props.EditEducation(index, 'title', e.target.value) }} value={item.title}/>
                    </div>
                    <div className="two fields">
                        <div className="field">
                            <label>Start date</label>
                            <div className="ui calendar" id="rangestart">
                                <div className="ui input left icon"> 
                                    <input type="date" onChange={(e) => { this.props.EditEducation(index, 'from', e.target.value) }} value={item.from}/>
                                </div>
                            </div>
                        </div>
                        <div className="field">
                            <label>End date</label>
                            <div className="ui calendar" id="rangeend">
                                <div className="ui input left icon">
                                    <input onChange={(e) => { this.props.EditEducation(index, 'to', e.target.value) }} type="date" value={item.to} />
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="field">
                        <label>Description</label>
                        <textarea rows="2" onChange={(e) => { this.props.EditEducation(index, 'description', e.target.value) }} value={item.description}></textarea>
                    </div>
                    <div className="comments">
                        <div className="content-space">
                            <FileDrop addFile={this.addFile} files={item.files}/>
                        </div>
                    </div>
                </form>
                <i onClick={ () => { this.props.DeleteEducation(index) } } className="trash alternate icon" style={{ cursor: 'pointer' }}></i>
            </StyledDiv>
        });
    }

    viewEducation = () => {
        return this.props.Education.education.map((item,index) => {
            var fromDate = new Date(item.from);
            var toDate = new Date(item.from);
            return <div key={index} className="content-width comment">
                <a href="#" className="company-image ui avatar image">
                    <AvatarImage className="avatar-image" src="data:image/png;base64,"/>
                </a>
                <div className="context-padding content-space">
                <a href="#" className="author">{item.title}</a>
                    <div className="metadata">
                        <span className="date">From {fromDate.toLocaleString('en-us',{month:'long', year:'numeric', day:'numeric'})} to {toDate.toLocaleString('en-us',{month:'long', year:'numeric', day:'numeric'})}</span>
                    </div>
                    <div className="text">
                        <p>{item.description}</p>
                    </div>
                </div>
            </div>;
        });
    }

    renderEducation() {
        if(this.context) {
            return this.renderEditableEducation();
        } else {
            return this.viewEducation();
        }
    }

    setEdit = () => {
        console.log('do nothing');
    }

    addEducationButton = () => {
        if(this.context) {
            return  <StyledComponent className="content-width comment">
                        <a href="#"className="ui avatar image">
                            <img className="avatar-image" src="https://d1nhio0ox7pgb.cloudfront.net/_img/o_collection_png/green_dark_grey/256x256/plain/plus.png"/>
                        </a>
                        <div className="context-padding content-space">
                            <a onClick={this.addGrade} className="author">Add Education</a>
                        </div>
                    </StyledComponent>
        }
    }

    render() {
        return (<Wrapper>
            <TitleRibbon title="Education"/>
            <div className="ui threaded comments">
                    {this.renderEducation()}
                    <StyledComponent className="content-width comment">
                        {this.addEducationButton()}
                    </StyledComponent>
            </div>
        </Wrapper>);
    }
}

const mapStateToProps = state => {
    return { Education: state.education } ;
}

export default connect(mapStateToProps, { AddEducation, EditEducation, DeleteEducation })(Education);